...
try:
	file = open("../automation-tools/lastprnums.txt", "r")
...
def save_pr_nums():
	...
	with open("../automation-tools/lastprnums.txt", "w") as file:
		file.write(content)

discord_webhook = "https://discordapp.com/api/webhooks/{}/{}".format(environ_vals[1], environ_vals[2])

# github api urls
list_prs_url = "http://api.github.com/repos/{}/pulls?state=closed&sort=created&direction=asc&base={}".format(environ_vals[0], "{}")
is_merged_url = "http://api.github.com/repos/{}/pulls/{}/merge".format(environ_vals[0], "{}")
...
class Changelog:
	...
	def dump_yaml(self, file_name):
		with open("../sealab13/html/changelogs/{}.yml".format(file_name), "w") as file:
			data = "author: \"{}\"\ndelete-after: true\nchanges:\n{}"
			change_data = ""
...
# CL regexps
# https://www.npmjs.com/package/github-username-regex (capture group for username)
cl_start = re.compile(":cl:\s*([a-z\d](?:[a-z\d]|-(?=[a-z\d])){0,38})?")
...
def generate_changelogs(branch):
	# get all closed prs on the branch
	resp = requests.post(list_prs_url.format(branch), auth=(environ_vals[3], environ_vals[4]))
	prs = resp.json()
	...
	for i in range(start, len(prs)):
		...
		resp = requests.post(is_merged_url.format(pr.get("number")), auth=(environ_vals[3], environ_vals[4]))
